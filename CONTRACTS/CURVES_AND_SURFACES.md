...
| `AtomicPolyline` | ordered tuple of points | `Polygon`, rounded `Rectangle` |
| `AtomicCircle` | plane + radius | `Circle`, `CircleCNR`, `Circle3Pt` |
| `AtomicArc` | plane + radius + angle interval | arc components |
| `AtomicEllipse` | plane + X/Y radii | ellipse authoring |
| `AtomicRectangle` | centered plane + side lengths | rectangle authoring |
| `AtomicInterpolatedCurve` | points the curve passes through | interpolated-curve authoring |
| `AtomicControlPointCurve` | points controlling a clamped curve | control-point-curve authoring |
| `AtomicNurbsCurve` | control points, weights, knots, degree | spline components, unification |

Every curve atom except `AtomicNurbsCurve` is an **authoring atom** — its
geometry is defined by compact, human-readable fields such as endpoints,
radii, interpolation points, or control points. They are cheap to create,
easy to inspect, and map directly to the types users expect.
...
Named solid authoring atoms may retain construction intent before conversion
to exact surface faces. `AtomicBox` stores a centered plane and X/Y/Z side
lengths; rendering and general Brep operations convert it to an exact
six-face `AtomicBrep`. Components that need box semantics, such as Box Array,
must consume `AtomicBox` rather than infer dimensions from a generic Brep.
...
```
Authoring Atoms            AtomicLine, AtomicPolyline, AtomicCircle, AtomicArc,
                           AtomicEllipse, AtomicRectangle, AtomicInterpolatedCurve,
...
| `AtomicEllipse` | 2 | 9 weighted CPs scaled in plane X/Y |
| `AtomicRectangle` | 1 | Closed 5-point polyline form |
| `AtomicInterpolatedCurve` | 3 | Deterministic cubic spans through authored points |
| `AtomicControlPointCurve` | clamped to point count | Open uniform NURBS |
...
from scratch: arbitrary-profile lofts, booleans, chamfers, fillets, and
shelling. Exact extrusions and lofts through already-compatible profiles can
be constructed directly as `AtomicSurface` values.
...
- Nine curve atom types with full NURBS unification.
- One surface atom type (`AtomicSurface`) with OCCT-style representation.
- Seven surface components (`CenterBox`, `Cone`, `Cylinder`, `Sphere`,
  `RuledSurface`, `Extrude`, and compatible-profile `Loft`) built directly
  as exact canonical geometry.
- Area and volume analysis use exact formulas for named primitives where
  available and knot-aware numerical integration for general NURBS geometry.
...
2. **Broaden Extrude** — use the adapter for cases that cannot be represented
   by the current direct tensor-product construction.

3. **Broaden Loft** — add loft options and accept incompatible profile
   structures after a future `make_compatible` unifier or kernel adapter can
   harmonize them.
